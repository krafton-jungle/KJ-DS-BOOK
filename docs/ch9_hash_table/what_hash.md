...
| 항목 | 설명 |
|------|------|
| [**해시(Hash)란 무엇인가?**](#hash_intro) | 해시 값과 해싱 기법의 정의 및 차이점 |
| [**왜 해싱이 필요한가?**](#why_hashing) | 배열 검색의 한계와 해싱의 O(1) 시간복잡도 장점 |
| [**해싱의 구성 요소**](#hash_components) | 키, 해시 함수, 해시 값, 해시 테이블, 충돌 처리 방식 |
| [**해시 함수(Hash Function)란 무엇인가?**](#hash_function) | 입력 데이터를 고정 크기 해시 값으로 변환하는 함수의 개념 |
| [**해시 함수의 특성**](#hash_properties) | 결정론적, 고정 출력, 효율성, 균등 분포 등 7가지 핵심 특성 |
| [**해시 함수의 종류**](#hash_types) | 나눗셈, 곱셈, 중앙제곱, 폴딩, 암호학적, 유니버설, 완전 해싱 |
| [**좋은 해시 함수란 무엇인가**](#good_hash_function) | 균등한 분포, 결정성, 빠른 계산, 충돌 최소화의 4가지 조건 |
| [**해시 함수 설계 시 고려사항**](#hash_design) | 입력 키 특성, 사용 목적, 충돌 허용 수준, 테이블 크기 선택 |
| [**해싱이 적합하지 않은 경우**](#when_not_to_use) | 정렬 유지, 접두사 검색, floor/ceiling 연산이 필요한 상황들 |
| [**이번 장을 마무리하며**](#conclusion) | 이론과 실무의 균형, 내장 해시 구현 활용 및 다음 장 예고 |
...
중앙 제곱법은 분포가 일정하지 않고, 입력 범위에 민감하여 실무에서는 잘 사용되지 않고, 폴딩 방법 또한 간단하지만 충돌 가능성 크기 때문에 소규모 키 집합엔 사용 가능하지만 일반적이지 않다.
...
## 이번 장을 마무리하며
이번 장에서는 다양한 해시 함수들과 그 특성을 살펴보았다.
실제로 시스템을 설계하거나 코드를 구현할 때는, 이론적 성능만큼이나 충돌 처리 방식, 입력 키의 특성, 그리고 언어/프레임워크에서 기본으로 제공되는 해시 구현 등을 함께 고려해야 한다.

예를 들어 Python의 dict나 Java의 HashMap, C++의 unordered_map은 내부적으로 자체 최적화된 해시 함수를 사용하므로, 대부분의 일반적인 사용 사례에서는 별도로 해시 함수를 구현할 필요가 없다. 그러나 대용량 데이터를 다루거나, 키가 매우 불균형한 경우, 혹은 해시 충돌이 보안 이슈가 될 수 있는 환경에서는 커스텀 해시 함수 또는 충돌 방지 설계가 필요할 수 있다.

다음 장에서는 해시 함수가 실제로 사용되는 해시 테이블 구조와, 실무에서 자주 문제로 등장하는 충돌 상황을 어떻게 처리하는지에 대해 구체적으로 다룬다. 