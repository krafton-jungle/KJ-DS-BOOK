...
그래프에서 간선은 정점 간의 연결보다 중요한 정보를 제공할 수 있다.
특히 DFS(Depth-First Search) 수행에서 간선은 탐색 순서와 구조에 따라 네 가지 유형으로 분류된다.

### 트리 간선(Tree edge)
트리 간선은 깊이 우선 탐색 중 아직 방문하지 않은 정점으로 이동할 때 생성되는 간선이다.
트리 간선들로 깊이 우선 탐색 트리를 구성하고, 이 트리는 탐색의 구조적 뼈대를 이룬다. 

### 역행 간선(Back Edge)
역행 간선은 이미 방문한 조상 정점으로 이동하는 간선이다.
탐색 경로 위에서 현재 정점의 상위 노드로 되돌아가는 구조를 가진다.
따라서 그래프 내에 사이클이 존재함을 알리는 단서가 된다.

### 순행 간선(Forward Edge)
순행 간선은 이미 방문한 자식 정점으로 이동하는 간선이다.
탐색 경로에서 하위 정점으로 이동하는 구조를 가진다.

### 교차 간선(Cross Edge)
교차 간선은 DFS 트리에서 다른 서브 트리 혹은 같은 레벨에 있는 다른 정점으로 향하는 간선이다. 